...
def run_pipeline(place: str | None = None, force_refresh: bool = False, use_ai: bool = False):
    ...
    if 'normalized_name:he' in osm_gdf.columns:
        # Use the Hebrew name if it exists, otherwise use the primary name
        osm_gdf['normalized_name'] = osm_gdf['normalized_name:he'].fillna(osm_gdf.get('normalized_osm_name'))
    ...
    print("--- OSM GDF columns before find_fuzzy_candidates ---")
    print(osm_gdf.columns)
    print("----------------------------------------------------")
