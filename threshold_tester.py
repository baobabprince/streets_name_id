...
דוגמאות:
  ...
  python test_threshold.py "אלעד" --ai-threshold 60
  python test_threshold.py "אלעד" --ai-threshold 60 --confident-threshold 80
"""
...
def test_threshold(osm_file, lamas_file, ai_threshold=50, confident_threshold=70):
    """
    ...
    for _, row in tqdm(unique_osm_names.iterrows(), total=len(unique_osm_names), desc="Calculating scores"):
        ...
        for _, lamas_row in lamas_df.iterrows():
            ...
            # Updated weights to match normalization.py
            weighted_score = (ratio * 0.05) + (token_sort * 0.05) + (token_set * 0.9)
...
def compare_thresholds(osm_file, lamas_file):
    ...
    """
    threshold_sets = [
        (90, 80, "Strict (Old Default)"),
        (70, 50, "Current Default"),
        (65, 50, "Slightly Relaxed Confident"),
        (60, 45, "More Relaxed"),
    ]
    ...
    # Optimization: Calculate for unique names only
    unique_osm_names = osm_df[['normalized_name']].drop_duplicates()
    print(f"Processing {len(unique_osm_names)} unique street names...")
    
    name_to_score = {}
    
    from tqdm import tqdm
    for _, row in tqdm(unique_osm_names.iterrows(), total=len(unique_osm_names)):
        osm_name = row['normalized_name']
        
        # Skip empty/NaN
        if pd.isna(osm_name) or str(osm_name).strip() == "":
            name_to_score[osm_name] = 0
            continue
        ...
        for _, lamas_row in lamas_df.iterrows():
            lamas_name = lamas_row['normalized_name']
            if pd.isna(lamas_name): continue
            lamas_name = str(lamas_name)
        ...
        name_to_score[osm_name] = best_score
    
    # Map back to all segments
    all_scores = []
    for _, row in osm_df.iterrows():
        osm_name = row['normalized_name']
        all_scores.append(name_to_score.get(osm_name, 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ai-threshold', type=float, default=50,
                        help='Minimum score for AI candidates (default: 50)')
    parser.add_argument('--confident-threshold', type=float, default=70,
                        help='Minimum score for confident matches (default: 70)')
