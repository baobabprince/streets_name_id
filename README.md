...
This project provides a comprehensive pipeline to map street names from OpenStreetMap (OSM) to their official LAMS (Local Authority Management System) IDs, leveraging fuzzy matching, topological analysis, and an optional AI-driven resolution for ambiguous cases. The system is designed to handle variations in street names, including abbreviations and synonyms, by using an official government data source that includes synonym information.
...
- **Fuzzy Matching:** Utilizes fuzzy logic to identify potential matches between OSM and LAMS street names, even when they are not identical.
...
- **Synonym Integration:** Leverages a LAMS data source that includes official street name synonyms, significantly improving match rates.
...
1.  **Data Acquisition:**
    *   Fetches all official street names and their synonyms from the `data.gov.il` API. This data includes a unique `official_code` for each street, which serves as the LAMS ID.
    *   Fetches all street geometries for a specified city (e.g., "בית שאן") from OpenStreetMap using the Overpass API.

2.  **Preprocessing and Normalization:**
    *   Both the OSM and LAMS street names are passed through a normalization function. This function expands common abbreviations (like `שד`, `רח`, `כי`), removes punctuation, and standardizes whitespace. This ensures that, for example, "שד' רוטשילד" and "שדרות רוטשילד" are treated as identical.
...
4.  **Candidate Matching:**
    *   For each street in the OSM dataset, the system performs a fuzzy match against all LAMS streets in the same city.
...
5.  **AI Resolution (Optional):**
    ...
    *   A detailed prompt is constructed, including the OSM street name, its adjacent streets (from the topology map), and the list of potential LAMS candidates with their fuzzy scores.
    *   The AI is instructed to act as a GIS expert and return the single best LAMS ID or 'None' if no candidate is a clear match.
...
-   **`lams_data.pkl`:** A cached pickle file of the raw data fetched from the LAMS API.
-   **`osm_data_{place}.pkl`:** A cached pickle file of the GeoDataFrame containing street data from OSM for the specified place.
-   **`final_mapping_{place}.csv`:** The main output file. This CSV contains the final mapping between `osm_id`, `osm_name`, and the matched `final_lams_id`.
